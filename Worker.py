...
# Import configs
from __config__ import AGENT_CACHE_FILE, PRIOR_AUTH_CONFIG_PATH
...
# Import prior auth post-call processor
try:
    from healthcare_rcm.processors.post_call_processor import process_completed_call
    PRIOR_AUTH_ENABLED = True
except ImportError:
    logger.warning("Prior auth processing module not found - feature disabled")
    PRIOR_AUTH_ENABLED = False
    process_completed_call = None
...
class AIAgent():
    ...
    def __init__(self, call_sid: str = str(uuid.uuid4()), agent_type: str = 'prior_auth', 
        ...
        agent_type (str): Type of agent (prior_auth, denial_mgmt, insurance_verify, claims_followup)
        agent_overrides (dict): Optional configuration overrides from API request
        """
        # Store agent type and overrides
        self.agent_type = agent_type
        self.agent_overrides = agent_overrides
        
        # For backwards compatibility - prior_auth_details alias
        self.prior_auth_details = agent_overrides if agent_type == 'prior_auth' else None
        
        # Initialize agent with merged config if overrides provided
        if agent_overrides:
            logger.info(f"[INIT] Initializing {agent_type} agent with API overrides")
            self.gpt = self._init_agent_with_overrides(agent_type, agent_overrides)
        else:
            logger.info(f"[INIT] Initializing {agent_type} agent with default config")
            self.gpt = self._init_agent_from_type(agent_type)
        
        # self.model : GPTPredictor = GPTPredictor()  # Disabled - optional feature, requires OpenAI
        self.model = None  # Filler prediction disabled

        self.stop_signal : bool = False
        self.is_first_audio_chunk_sent: bool = False
        ...
        # Flag to signal call should end
    ...
    def _init_agent_from_type(self, agent_type: str) -> ConversationalModel:
        """
        Initialize agent from type without overrides
        
        Args:
            agent_type: Type of agent to initialize
        
        Returns:
            Initialized ConversationalModel
        """
        from __config__ import get_agent_config
        
        agent_config = get_agent_config(agent_type)
        config_path = agent_config['config_path']
        
        logger.info(f"[CONFIG] Loading config from: {config_path}")
        return ConversationalModel(config_path=config_path).init_agent()
    
    def _init_agent_with_overrides(self, agent_type: str, overrides: Dict) -> ConversationalModel:
        """
        Initialize agent with merged configuration (base config + API overrides)
        
        Args:
            agent_type: Type of agent (prior_auth, denial_mgmt, etc.)
            overrides: Dictionary with values from API request (patient_name, etc.)
        
        Returns:
            Initialized ConversationalModel with merged config
        """
        from __config__ import get_agent_config
        import json
        
        # Get config path for agent type
        agent_config = get_agent_config(agent_type)
        config_path = agent_config['config_path']
        ...
        # Merge overrides (API values take precedence)
        merged_config = {**base_config, **overrides}
        
        # Log what we're overriding
        overridden_keys = [k for k in overrides.keys() if k in base_config and overrides[k] != base_config.get(k)]
        if overridden_keys:
            logger.info(f"[CONFIG] Overriding {len(overridden_keys)} fields from API: {overridden_keys}")
            for key in overridden_keys:
                logger.debug(f"  {key}: {base_config.get(key)} -> {overrides[key]}")
        
        # Initialize ConversationalModel with merged config
        # Pass all config keys as kwargs to ConversationalModel.__init__
        return ConversationalModel(**merged_config).init_agent()
        ...
        async def output_iterator():
            if self.use_cache:
                ...
                async for chunk in self.streaming_generator:
                    # Handle both BedrockClaude (plain string) and OpenAI (object) formats
...
################################################ UPDATING AGENT FOR CONVERSATION FLOW ###############################################
    def update_agent_response(self):
        ...
        # Check if we should trigger post-call processing
        if PRIOR_AUTH_ENABLED and self.should_trigger_post_call_processing(self.ai_response):
            logger.info(f"[{self.agent_type.upper()}] Scheduling post-call processing...")
            self.should_end_call = True
            logger.info(f"[{self.agent_type.upper()}] Call will be disconnected")
            # Schedule as a background task (don't block the call)
            asyncio.create_task(self.process_completed_call())
...
################################################ POST-CALL PROCESSING ###############################################
    async def process_completed_call(self):
        """
        Process call after completion based on agent type
        Extracts, validates, and stores structured information from the conversation
        """
        if not PRIOR_AUTH_ENABLED:
            logger.warning("Post-call processing is disabled")
            return None
        
        try:
            logger.info(f"[{self.agent_type.upper()}] Starting post-call processing...")
            
            # Get conversation history
            conversation_history = self.gpt.conversation_history.copy()
            
            if not conversation_history or len(conversation_history) < 3:
                logger.warning(f"Conversation too short for {self.agent_type} processing")
                return None
            ...
            if self.agent_type == 'prior_auth':
                record = await self._process_prior_auth(conversation_history, agent_config)
            elif self.agent_type == 'denial_mgmt':
                record = await self._process_denial_mgmt(conversation_history, agent_config)
            else:
                logger.warning(f"Unknown agent type for processing: {self.agent_type}")
                return None
            
            if record:
                logger.info(f"[SUCCESS] {self.agent_type} call processed successfully")
            
            return record
            
        except Exception as e:
            logger.error(f"Error in {self.agent_type} post-call processing: {e}", exc_info=True)
            return None
    
    async def _process_prior_auth(self, conversation_history, agent_config):
        """Process prior authorization call"""
        from healthcare_rcm.processors.post_call_processor import process_completed_call
        ...
        if record:
            logger.info(f"   Status: {record.authorization.status.value}")
            logger.info(f"   Outcome: {record.call_outcome.value}")
            if record.authorization.authorization_number:
                logger.info(f"   Auth Number: {record.authorization.authorization_number}")
        
        return record
    
    async def _process_denial_mgmt(self, conversation_history, agent_config):
        """Process denial management call"""
        try:
            import boto3
            from healthcare_rcm.extractors.denial_mgmt_extractor import DenialMgmtExtractor
            from healthcare_rcm.storage.prior_auth_storage import PriorAuthStorage
            
            # Initialize Bedrock client
            bedrock_client = boto3.client('bedrock-runtime', region_name='us-east-1')
            
            # Extract information
            extractor = DenialMgmtExtractor(bedrock_client=bedrock_client)
            record = extractor.extract_from_conversation(
                conversation_history=conversation_history,
                call_id=self.call_sid,
                agent_config=agent_config
            )
            ...
            # Store record
            storage = PriorAuthStorage(base_dir='denial_mgmt_records')
            
            # Determine status based on resolution
            status = record['denial_info']['resolution_status']
            if status in ['overturned', 'resubmit_required', 'pending_documentation']:
                folder = 'success'
            elif status in ['upheld']:
                folder = 'failed'
            else:
                folder = 'pending'
            
            # Save record
            storage.save_record(record, status=folder, call_id=self.call_sid)
            
            logger.info(f"   Resolution Status: {status}")
            logger.info(f"   Reference: {record.get('reference_number')}")
            
            return record
            
        except Exception as e:
            logger.error(f"Error processing denial mgmt call: {e}", exc_info=True)
            return None
    
    def _build_agent_config(self) -> dict:
        """Build agent configuration dictionary for post-call processing"""
        try:
            # PRIORITY 1: Use agent_overrides from API request if available
            if self.agent_overrides:
                logger.info(f"Using {self.agent_type} details from API request")
                return self.agent_overrides.copy()
            
            # PRIORITY 2: Try to extract config from ConversationalModel
            config = {}
            
            # Get agent role
            if hasattr(self.gpt, 'agent_role'):
                config['agent_role'] = self.gpt.agent_role
            
            # Try to get config from the ConversationalModel
            if hasattr(self.gpt, 'config') and isinstance(self.gpt.config, dict):
                # Extract patient info
                config['patient_name'] = self.gpt.config.get('patient_name', 'Unknown')
                config['patient_dob'] = self.gpt.config.get('patient_dob')
                config['member_id'] = self.gpt.config.get('member_id')
                
                # Extract provider info
                config['provider_name'] = self.gpt.config.get('provider_name', 'Unknown')
                config['provider_npi'] = self.gpt.config.get('provider_npi')
                
                # Extract procedure info
                config['cpt_code'] = self.gpt.config.get('cpt_code', 'UNKNOWN')
                config['procedure_description'] = self.gpt.config.get('procedure_description')
                config['icd_code'] = self.gpt.config.get('icd_code')
                config['diagnosis_description'] = self.gpt.config.get('diagnosis_description')
                
                # Extract insurance info
                config['insurance_company'] = self.gpt.config.get('insurance_company', 'Unknown Insurance')
            
            logger.debug(f"Built agent config: {list(config.keys())}")
            return config
            
        except Exception as e:
            logger.error(f"Error building agent config: {e}")
            return {'agent_role': 'unknown'}
    
    def should_trigger_post_call_processing(self, response_text: str) -> bool:
        """
        Check if post-call processing should be triggered
        
        Args:
            response_text: The AI agent's response text
        
        Returns:
            True if post-call processing should be triggered
        """
        # Check for END_OF_CALL marker
        if "<END_OF_CALL>" in response_text:
            logger.info("[END_OF_CALL] Detected - will trigger post-call processing")
            return True
        
        # Check for other indicators of call completion
        completion_phrases = [
            "goodbye",
            "have a good day",
            "thank you for your time",
            "call has been concluded",
            "ending the call"
        ]
        ...
        for phrase in completion_phrases:
            if phrase in response_lower:
                logger.info(f"📞 Call completion phrase detected: '{phrase}'")
                return True
