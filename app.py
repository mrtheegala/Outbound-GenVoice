...
class CallRequest(BaseModel): 
    ...
    welcome_message: str = "Hi my names is myra, and I'm calling from ascent health care."
    to_number: str = "+919550760205"  # Default number, can be overridden in request
    agent_type: str = "prior_auth"  # Options: prior_auth, denial_mgmt, insurance_verify, claims_followup
    
    # Prior authorization details (optional)
    ...
    # Denial management details (optional)
...
################################################ HELPER FUNCTIONS #####################################################
def get_agent(call_sid: str, prior_auth_details=None):
    """
    Retrieves the AIAgent instance associated with the given call_sid.

    Args:
    call_sid (str): The call SID to retrieve the agent for.
    prior_auth_details (dict): Optional prior authorization details

    Returns:
    AIAgent: The AIAgent instance associated with the call_sid.
    """
    agent_retrieval_start_time = time.time()
    if call_sid not in agents:
        agents[call_sid] = AIAgent(call_sid, prior_auth_details=prior_auth_details)
...
async def audio_stream(websocket: WebSocket, call_sid: str):
    ...
    async def play_welcome_message():
        ...
        try:
            if agent.welcome_file_path is None:
                logger.info("No welcome audio file available, skipping welcome message playback...")
                return
                    ...
                    try:
                        ...
                        while True:
                            ...
                            if agent.should_end_call:
                                logger.info("[CALL END] Agent requested call termination")
...
async def make_call(call_request: CallRequest):
    ...
    # Build agent overrides based on agent type
    agent_overrides = None
    agent_type = call_request.agent_type
    
    if agent_type == 'prior_auth':
        # Build prior auth details if provided
        if call_request.patient_name:
            agent_overrides = {
                'patient_name': call_request.patient_name,
                'patient_dob': call_request.patient_dob,
                'member_id': call_request.member_id,
                'provider_name': call_request.provider_name,
                'provider_npi': call_request.provider_npi,
                'cpt_code': call_request.cpt_code or 'procedure_code',
                'procedure_code': call_request.cpt_code,
                'procedure_name': call_request.procedure_description or 'procedure_name',
                'procedure_description': call_request.procedure_description,
                'icd_code': call_request.icd_code or 'diagnosis_code',
                'diagnosis_code': call_request.icd_code,
                'diagnosis_description': call_request.diagnosis_description,
                'insurance_company': call_request.insurance_company or 'payer_name',
                'payer_name': call_request.insurance_company,
                'proposed_date': call_request.proposed_date,
                'urgency_level': call_request.urgency_level,
                'clinical_notes': call_request.clinical_notes
            }
            logger.info(f"[API] Prior auth details received for patient: {call_request.patient_name}")
    
    elif agent_type == 'denial_mgmt':
        # Build denial management details if provided
        if call_request.claim_number:
            agent_overrides = {
                'claim_number': call_request.claim_number,
                'patient_name': call_request.patient_name,
                'service_date': call_request.service_date,
                'procedure_code': call_request.procedure_code or call_request.cpt_code,
                'procedure_name': call_request.procedure_name or call_request.procedure_description,
                'diagnosis_code': call_request.diagnosis_code or call_request.icd_code,
                'denial_code': call_request.denial_code,
                'denial_reason': call_request.denial_reason,
                'payer_name': call_request.payer_name or call_request.insurance_company,
                'provider_name': call_request.provider_name,
                'provider_npi': call_request.provider_npi
            }
            logger.info(f"[API] Denial mgmt details received for claim: {call_request.claim_number}")
    
    # Initialize agent with type and overrides (CRITICAL: must be done BEFORE agent starts)
    ai_agent = AIAgent(agent_type=agent_type, agent_overrides=agent_overrides)
    ...
    if welcome_message != "":
        ...
        try:
            ...
            logger.warning(f"Could not generate welcome audio (ffmpeg required): {e}")
            logger.info("Continuing without welcome message audio...")
            ai_agent.welcome_file_path = None
    else:
